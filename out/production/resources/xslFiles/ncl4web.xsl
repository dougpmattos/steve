...
<xsl:namespace-alias stylesheet-prefix="e" result-prefix="#default"/>
...
<style>
.innerRegion{ position:absolute; height:100%; width:100%; z-index:auto; } .stoped{ display:none; visibility:hidden; } .started{ display:inline; visibility:visible; } .active *{ margin-left:-4px; border-width:2px; border-color:blue; border-style: solid; }
</style>
<!--
...
<script src="http://www.midiacom.uff.br/~caleb/ncl4web/ncl-complements.js"></script>
<script>
...
<xsl:variable name="docImp" select="document(@documentURI)"/>
...
transicoes["
<xsl:value-of select="concat($alias,'_',@id)"/>
"] = {dur:"
<xsl:value-of select="@dur"/>
",type:"
<xsl:value-of select="@type"/>
...
<xsl:variable name="alias" select="/ncl/head/ruleBase/importBase/@alias|/e:ncl/e:head/e:ruleBase/e:importBase/@alias"/>
...
<xsl:if test="$alias!=''">
<xsl:value-of select="concat($alias,'_')"/>
</xsl:if>
...
<xsl:with-param name="searchId" select="$descritor/@transIn"/>
...
<xsl:call-template name="encontraNaBase">
<xsl:with-param name="searchId" select="$descritor/@transOut"/>
<xsl:with-param name="originalDoc" select="$originalDoc"/>
<xsl:with-param name="baseName">transition</xsl:with-param>
</xsl:call-template>
</xsl:attribute>
...
<xsl:param name="regiao"/>
<xsl:param name="alias"/>
<xsl:param name="regionURI"/>
<xsl:param name="originalDoc"/>
<div>
<xsl:attribute name="id">
<xsl:if test="$alias!=''">
...
<xsl:value-of select="$regiao/@left"/>
<xsl:if test="not(contains($regiao/@left,'%'))">
...
<xsl:value-of select="$regiao/@right"/>
<xsl:if test="not(contains($regiao/@right,'%'))">
<xsl:text>px</xsl:text>
</xsl:if>
<xsl:text>;</xsl:text>
</xsl:if>
...
<xsl:value-of select="$regiao/@top"/>
<xsl:if test="not(contains($regiao/@top,'%'))">
<xsl:text>px</xsl:text>
</xsl:if>
<xsl:text>;</xsl:text>
</xsl:if>
...
<xsl:value-of select="$regiao/@bottom"/>
<xsl:if test="not(contains($regiao/@bottom,'%'))">
<xsl:text>px</xsl:text>
</xsl:if>
<xsl:text>;</xsl:text>
</xsl:if>
<xsl:text>height:</xsl:text>
<xsl:if test="count($regiao/@height) = 0">100%</xsl:if>
<xsl:if test="$regiao/@height != ''">
<xsl:value-of select="$regiao/@height"/>
<xsl:if test="not(contains(@height,'%'))">
<xsl:text>px</xsl:text>
</xsl:if>
</xsl:if>
<xsl:text>;</xsl:text>
<xsl:text>width:</xsl:text>
<xsl:if test="count($regiao/@width) = 0">100%</xsl:if>
<xsl:if test="$regiao/@width != ''">
<xsl:value-of select="$regiao/@width"/>
...
<xsl:value-of select="number($regiao/@zIndex)*1000"/>
<xsl:text>;</xsl:text>
</xsl:attribute>
</xsl:if>
...
<xsl:with-param name="descritor" select="."/>
<xsl:with-param name="originalDoc" select="$originalDoc"/>
</xsl:call-template>
</xsl:for-each>
<xsl:if test="$alias!=''">
...
<xsl:with-param name="descritor" select="."/>
<xsl:with-param name="originalDoc" select="$originalDoc"/>
<xsl:with-param name="alias" select="$aliasDes"/>
</xsl:call-template>
</xsl:for-each>
</xsl:if>
</xsl:for-each>
...
<xsl:with-param name="regiao" select="."/>
<xsl:with-param name="alias" select="$alias"/>
...
<xsl:variable name="regionURI" select="@documentURI"/>
...
<xsl:call-template name="interpretaDescritor">
<xsl:with-param name="descritor" select="."/>
<xsl:with-param name="originalDoc" select="$originalDoc/ncl/head"/>
<xsl:with-param name="alias" select="$aliasDes"/>
</xsl:call-template>
</xsl:for-each>
...
<xsl:with-param name="descritorSwitch" select="."/>
<xsl:with-param name="originalDoc" select="$originalDoc"/>
<xsl:with-param name="alias" select="$aliasDes"/>
</xsl:call-template>
</xsl:for-each>
...
<xsl:with-param name="oelo" select="."/>
</xsl:call-template>
</xsl:for-each>
</script>
</body>
...
<xsl:value-of select="concat(@alias,'_',$searchId)"/>
</xsl:if>
</xsl:for-each>
</xsl:when>
<xsl:otherwise>
...
<xsl:value-of select="concat(@alias,'_',substring-after($searchId,'#'))"/>
</xsl:if>
</xsl:for-each>
</xsl:when>
<xsl:otherwise>
...
<xsl:value-of select="substring-after($searchId,'#')"/>
</xsl:if>
</xsl:otherwise>
</xsl:choose>
</xsl:otherwise>
...
<xsl:value-of select="concat(substring-before($searchId,'#'),'_',substring-after($searchId,'#'))"/>
</xsl:when>
<xsl:otherwise>
...
<xsl:value-of select="$descritorSwitch/@id"/>
</xsl:variable>
...
<xsl:call-template name="encontraNaBase">
<xsl:with-param name="searchId" select="@rule"/>
<xsl:with-param name="originalDoc" select="$originalDoc"/>
<xsl:with-param name="baseName">rule</xsl:with-param>
</xsl:call-template>
</xsl:variable>
<xsl:choose>
<xsl:when test="$regra!=''">
if(window['
<xsl:value-of select="$regra"/>
']()){
</xsl:when>
<xsl:otherwise>
<xsl:choose>
<xsl:when test="contains($descritorSwitch/@rule,'#')">
<xsl:variable name="docRule" select="document(//ruleBase/importBase[@alias=substring-before($descritorSwitch/@rule,'#')]|//e:ruleBase/e:importBase[@alias=substring-before($descritorSwitch/@rule,'#')])"/>
<xsl:variable name="ruleNova" select="$docRule/ncl/head/ruleBase/rule[@alias=substring-after($descritorSwitch/@rule,'#')]|$docRule/e:ncl/e:head/e:ruleBase/e:rule[@alias=substring-after($descritorSwitch/@rule,'#')]"/>
if($('span.settings>input.property[name="
<xsl:value-of select="$ruleNova/@var"/>
"]').attr('value')
<xsl:call-template name="pegaComparador">
<xsl:with-param name="comparator" select="$ruleNova/@comparator "/>
</xsl:call-template>
'
<xsl:value-of select="$ruleNova/@value"/>
'){
</xsl:when>
<xsl:otherwise>
<xsl:variable name="ruleNova" select="//rule[@alias=substring-after($descritorSwitch/@rule,'#')]|//e:rule[@alias=substring-after($descritorSwitch/@rule,'#')]"/>
if($('span.settings>input.property[name="
<xsl:value-of select="$ruleNova/@var"/>
"]').attr('value')
<xsl:call-template name="pegaComparador">
...
<script>
<xsl:value-of select="$descId"/>
(); $('span.settings').bind('onEndAttribution',
<xsl:value-of select="$descId"/>
);
</script>
</xsl:template>
...
<xsl:with-param name="id" select="$regra/@id"/>
<xsl:with-param name="alias" select="$alias"/>
</xsl:call-template>
</xsl:variable>
...
<xsl:value-of select="concat(@id,'()',$operador)"/>
</xsl:for-each>
<xsl:value-of select="$endComp"/>
</xsl:variable>
return
<xsl:value-of select="$stringCondicao"/>
; }
</xsl:template>
...
function
<xsl:value-of select="$nomeRega"/>
(){ variavel = $('span.settings>input.property[name="
<xsl:value-of select="$regra/@var"/>
"]').attr('value'); valor = '
<xsl:value-of select="$regra/@value"/>
'; return variavel
<xsl:call-template name="pegaComparador">
<xsl:with-param name="comparator" select="$regra/@comparator "/>
</xsl:call-template>
valor; }
</xsl:template>
...
<xsl:value-of select="concat(substring-before(@role,'#'),'_',substring-after(@role,'#'))"/>
</xsl:when>
<xsl:otherwise>
...
<xsl:value-of select="concat(substring-before($vinculoRegra/@rule,'#'),'_',substring-after($vinculoRegra/@rule,'#'))"/>
</xsl:when>
<xsl:otherwise>
...
<xsl:with-param name="delay" select="$connector/compoundCondition/@delay|$connector/e:compoundCondition/@delay"/>
...
<xsl:when test="$tempo != ''">
function
<xsl:value-of select="concat($functionName,'__delayed')"/>
(){
<xsl:value-of select="$restoDaAcao"/>
} setTimeout("
<xsl:value-of select="concat($functionName,'__delayed')"/>
()",
...
<xsl:value-of select="$no"/>
<xsl:text>','</xsl:text>
<xsl:value-of select="$connector/simpleAction[@role='set']/@value|$connector/e:simpleAction[@role='set']/@value"/>
<xsl:text>'</xsl:text>
<xsl:if test="@interface != ''">
<xsl:text>,'</xsl:text>
<xsl:value-of select="@interface"/>
...
<xsl:value-of select="$connector/simpleAction/@repeat|$connector/e:simpleAction/@repeat"/>
);
<xsl:if test="count($connector/simpleAction/@repeatDelay|$connector/e:simpleAction/@repeatDelay)>0">
<xsl:variable name="repeatDelay" select="connector/simpleAction/@repeatDelay|$connector/e:simpleAction/@repeatDelay"/>
...
<xsl:value-of select="substring-before(bindParam[@name=substring-after($repeatDelay,'$')]/@value|e:bindParam[@name=substring-after($repeatDelay,'$')]/@value,'s')"/>
</xsl:when>
<xsl:otherwise>
<xsl:value-of select="substring-before($elo/linkParam[@name=substring-after($repeatDelay,'$')]/@value|$elo/e:linkParam[@name=substring-after($repeatDelay,'$')]/@repeatDelay,'s')"/>
</xsl:otherwise>
</xsl:choose>
</xsl:when>
<xsl:otherwise>
<xsl:value-of select="$repeatDelay"/>
</xsl:otherwise>
</xsl:choose>
</xsl:variable>
$(
...
<xsl:value-of select="$rpDelay"/>
);
</xsl:if>
</xsl:if>
<!-- finalmente acha a linha de execução da funçao -->
<xsl:variable name="tempo">
<xsl:variable name="delay" select="$connector/simpleAction/@delay|$connector/e:simpleAction/@delay"/>
<xsl:call-template name="pegaDelay">
<xsl:with-param name="delay" select="$delay"/>
<xsl:with-param name="elo" select="$elo"/>
<xsl:with-param name="bindParam" select="bindParam[@name=substring-after($delay,'$')]|e:bindParam[@name=substring-after($delay,'$')]"/>
</xsl:call-template>
</xsl:variable>
...
<xsl:text>setTimeout("</xsl:text>
<xsl:value-of select="$acaoJava"/>
<xsl:text>",</xsl:text>
<xsl:value-of select="$tempo"/>
*1000);
</xsl:when>
...
$(document).bind('
<xsl:value-of select="concat($functionName,$role,'abc_123')"/>
',function(){
...
<xsl:if test="$interaction">});</xsl:if>
</xsl:for-each>
<xsl:if test="$interaction">
$(document).bind('
<xsl:value-of select="$functionName"/>
...
<xsl:text>$('input.property[parent="</xsl:text>
<xsl:value-of select="$no"/>
<xsl:text>"][name="</xsl:text>
<xsl:value-of select="$elo/bind[@role=$assaement/@role]/@interface|$elo/e:bind[@role=$assaement/@role]/@interface"/>
<xsl:text>"]').attr('value');</xsl:text>
</xsl:when>
<xsl:otherwise>
<xsl:variable name="interface" select="$elo/bind[@role=$assaement/@role]/@interface"/>
<xsl:text>$('input.property[parent="</xsl:text>
<xsl:value-of select="$originalDoc//port[@id=$interface]/@component|$originalDoc//e:port[@id=$interface]/@component"/>
<xsl:text>"][name="</xsl:text>
<xsl:value-of select="$originalDoc//port[@id=$interface]/@interface|$originalDoc//e:port[@id=$interface]/@inteface"/>
<xsl:text>"]').attr('value');</xsl:text>
</xsl:otherwise>
</xsl:choose>
</xsl:when>
...
<xsl:text>$('#'+$('#</xsl:text>
<xsl:value-of select="$no"/>
<xsl:text>').attr('value')).attr('state');</xsl:text>
</xsl:when>
<xsl:otherwise>
<!-- é um nó -->
<xsl:text>$('#</xsl:text>
<xsl:value-of select="$no"/>
<xsl:text>').attr('state');</xsl:text>
</xsl:otherwise>
</xsl:choose>
</xsl:when>
</xsl:choose>
...
<xsl:variable name="nomeAss" select="substring-after($assaement/@value,'$')"/>
<xsl:choose>
<xsl:when test="count($elo/e:bind[@role=$role]/e:bindParam[@name=$nomeAss]|$elo/bind[@role=$role]/bindParam[@name=$nomeAss])>0">
<xsl:text>'</xsl:text>
<xsl:value-of select="$elo/e:bind[@role=$role]/e:bindParam[@name=$nomeAss]/@value|$elo/bind[@role=$role]/bindParam[@name=$nomeAss]/@value"/>
<xsl:text>';</xsl:text>
</xsl:when>
<xsl:otherwise>
<xsl:text>'</xsl:text>
<xsl:value-of select="$elo/e:linkParam[@name=$nomeAss]/@value|$elo/linkParam[@name=$nomeAss]/@value"/>
<xsl:text>';</xsl:text>
</xsl:otherwise>
</xsl:choose>
</xsl:when>
<xsl:otherwise>
<xsl:text>'</xsl:text>
<xsl:value-of select="$assaement/@value"/>
<xsl:text>';</xsl:text>
</xsl:otherwise>
</xsl:choose>
</xsl:otherwise>
...
<xsl:param name="elo"/>
<xsl:param name="bindParam"/>
<xsl:choose>
<xsl:when test="contains($delay,'$')">
<xsl:choose>
<xsl:when test="count($bindParam)> 0">
<xsl:value-of select="substring-before($bindParam[@name=substring-after($delay,'$')]/@value,'s')"/>
</xsl:when>
<xsl:otherwise>
<xsl:value-of select="substring-before($elo/linkParam[@name=substring-after($delay,'$')]/@value|$elo/e:linkParam[@name=substring-after($delay,'$')]/@value,'s')"/>
</xsl:otherwise>
</xsl:choose>
</xsl:when>
<xsl:otherwise>
<xsl:value-of select="substring-before($delay,'s')"/>
</xsl:otherwise>
</xsl:choose>
</xsl:template>
...
<xsl:value-of select="concat($functionName,$interaction)"/>
= {number:
<xsl:value-of select="count($connector/e:compoundCondition/e:simpleCondition|$connector/compoundCondition/simpleCondition)+count($connector/compoundCondition/compoundCondition|$connector/e:compoundCondition/e:compoundCondition)"/>
,string:"",counter:0}; function
<xsl:value-of select="concat($functionName,'_',$interaction)"/>
(origin,target){ if(origin.data.id == target){
...
comparado =
<xsl:call-template name="interpretaAssassement">
<xsl:with-param name="assaement" select="*[1]"/>
<xsl:with-param name="elo" select="$elo"/>
<xsl:with-param name="role" select="*[2]/@role"/>
<xsl:with-param name="originalDoc" select="$originalDoc"/>
</xsl:call-template>
valor =
<xsl:call-template name="interpretaAssassement">
<xsl:with-param name="assaement" select="*[2]"/>
<xsl:with-param name="role" select="*[1]/@role"/>
<xsl:with-param name="elo" select="$elo"/>
<xsl:with-param name="originalDoc" select="$originalDoc"/>
</xsl:call-template>
<xsl:variable name="comparador" select="$connector/e:compoundCondition/e:assessmentStatement/@comparator|$connector/compoundCondition/assessmentStatement/@comparator"/>
if(!(comparado
<xsl:call-template name="pegaComparador">
<xsl:with-param name="comparator" select="$comparador "/>
</xsl:call-template>
valor)) return;
</xsl:for-each>
} else{ return; }
...
<xsl:text>setTimeout("</xsl:text>
<xsl:value-of select="$functionName"/>
<xsl:text>({data:{id:"+origin.data.id+"}},"+target+")",</xsl:text>
<xsl:value-of select="$tempo"/>
*1000);
</xsl:when>
<xsl:otherwise>
...
if(origin.stopImmediatePropagation) origin.stopImmediatePropagation(); return true; } }
...
<xsl:variable name="role" select="$connector/simpleCondition/@role|$connector/e:simpleCondition/@role"/>
<xsl:for-each select="$elo/bind[@role=$role]|$elo/e:bind[@role=$role]">
...
<xsl:variable name="tempo">
<xsl:variable name="delay" select="$connector/simpleCondition/@delay|$connector/e:simpleCondition/@delay"/>
<xsl:call-template name="pegaDelay">
<xsl:with-param name="delay" select="$delay"/>
<xsl:with-param name="elo" select="$elo"/>
<xsl:with-param name="bindParam" select="bindParam[@name=substring-after($delay,'$')]|e:bindParam[@name=substring-after($delay,'$')]"/>
...
<xsl:value-of select="bindParam[@name=substring-after($key,'$')]/@value|e:bindParam[@name=substring-after($key,'$')]/@value"/>
</xsl:when>
<xsl:when test="count($elo/linkParam[@name=substring-after($key,'$')]|$elo/e:linkParam[@name=substring-after($key,'$')])>0">
<xsl:value-of select="$elo/linkParam[@name=substring-after($key,'$')]/@value|$elo/e:linkParam[@name=substring-after($key,'$')]/@value"/>
</xsl:when>
</xsl:choose>
</xsl:when>
<xsl:otherwise>
...
<xsl:variable name="component" select="@component"/>
<xsl:if test="$chave != ''">
hasNclKeys = true; nclKeys['
<xsl:value-of select="$chave"/>
'] = ""; nclKeys['
<xsl:value-of select="$chave"/>
Name'] = "não selecionada"; function
<xsl:value-of select="$functionName"/>
_
<xsl:value-of select="$chave"/>
(origin,target){ if($('#
<xsl:value-of select="$componentInterface"/>
').attr('state')!='occurring') return false; if($('.started#'+origin.data.id).length>0) if(
<xsl:value-of select="$functionName"/>
({data:{id:origin.data.id}},origin.data.id))	origin.stopImmediatePropagation(); }
<xsl:choose>
<xsl:when test="$tempo != ''">
function
<xsl:value-of select="concat($functionName,'_SimpleCondDelay')"/>
(origin,target){
<xsl:text>setTimeout("</xsl:text>
<xsl:value-of select="$functionName"/>
<xsl:text>({data:{id:"+origin.data.id+"}},"+target+")",</xsl:text>
...
<xsl:value-of select="$oelo/@id"/>
</xsl:when>
<xsl:otherwise>
<xsl:for-each select="$oelo/bind|$oelo/e:bind">
<xsl:variable name="roleComp">
<xsl:if test="count(@interface) = 1">
<xsl:value-of select="concat(@role,@component,@interface,'_')"/>
</xsl:if>
<xsl:if test="count(@interface) = 0">
<xsl:value-of select="concat(@role,@component,'_')"/>
</xsl:if>
</xsl:variable>
<xsl:if test="contains($roleComp,'.')">
<xsl:value-of select="concat(substring-before($roleComp,'.'),'_',substring-after($roleComp,'.'))"/>
</xsl:if>
<xsl:if test="not(contains($roleComp,'.'))">
<xsl:value-of select="$roleComp"/>
</xsl:if>
</xsl:for-each>
</xsl:otherwise>
</xsl:choose>
...
<xsl:value-of select="concat(substring-before(@xconnector,'.'),'_',substring-after(@xconnector,'.'))"/>
</xsl:when>
<xsl:otherwise>
<xsl:value-of select="@xconnector"/>
</xsl:otherwise>
</xsl:choose>
</xsl:variable>
<xsl:choose>
<xsl:when test="contains($xconector,'#')">
<xsl:value-of select="concat(substring-before($xconector,'#'),substring-after($xconector,'#'))"/>
</xsl:when>
<xsl:otherwise>
<xsl:value-of select="$xconector"/>
</xsl:otherwise>
</xsl:choose>
</xsl:variable>
<xsl:choose>
<xsl:when test="contains($oelo/@xconnector,'#')">
<xsl:variable name="connectorID" select="substring-after($oelo/@xconnector,'#')"/>
<xsl:variable name="alias" select="substring-before($oelo/@xconnector,'#')"/>
...
<xsl:with-param name="connector" select="$conectorExterno/ncl/head/connectorBase/causalConnector[@id=$connectorID]|$conectorExterno/e:ncl/e:head/e:connectorBase/e:causalConnector[@id=$connectorID]"/>
<xsl:with-param name="functionName" select="$functionName"/>
<xsl:with-param name="oelo" select="$oelo"/>
</xsl:call-template>
</xsl:when>
<xsl:otherwise>
<xsl:variable name="connectorID" select="@xconnector"/>
<xsl:call-template name="intepretaElo2">
<xsl:with-param name="connector" select="//causalConnector[@id=$connectorID]|//e:causalConnector[@id=$connectorID]"/>
<xsl:with-param name="functionName" select="$functionName"/>
<xsl:with-param name="oelo" select="$oelo"/>
</xsl:call-template>
</xsl:otherwise>
...
if($('#'+target+'.started,input.port#'+target).length ==0){ return; }
</xsl:if>
...
<xsl:value-of select="substring-before(@begin,'s')"/>
</xsl:when>
<xsl:otherwise>
...
<xsl:value-of select="substring-before(@end,'s')"/>
</xsl:when>
<xsl:otherwise>
